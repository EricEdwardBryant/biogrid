...
#' 
plot.tbl_biogrid_aggregated <- function(
  ...
  if (attr(x, 'net') == 'outer') {
    ...
    if (length(which(cols)) < 2) {
      stop('No interactors met the density threshold of ', density, 
           ...
           '  See ?plot.tbl_biogrid_aggregated for more help.')
    }
    mat <- mat[, cols]
  }
